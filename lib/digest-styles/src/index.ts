/**
 * Single source of truth for digest style presets.
 *
 * Imported by both:
 *  - the API server PDF renderer (artifacts/api-server)
 *  - the client style-picker preview (artifacts/trip-correspondent)
 *
 * Edit styles here — changes are reflected in the PDF and the UI automatically.
 */
...
export type DigestStylePreset = {
  id: DigestStyleId;
  name: string;
  /** Short description shown in the style picker UI */
  description: string;
...
/** Ordered array of style presets, suitable for rendering a picker list. */
export const DIGEST_STYLES_LIST: DigestStylePreset[] = DIGEST_STYLE_IDS.map(
  (id) => DIGEST_STYLES[id],
);
